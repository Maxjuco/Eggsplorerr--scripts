...
public class S_AIController : MonoBehaviour
{
    ...
    public LayerMask TargetMask;                    //  To detect the player with the raycast
    ...
    private void HandleAnimator()
    {
        ...
        if((navMeshAgent.speed == speedWalk) && !isWalking)
        {
            animator.SetBool("isWalking", true);
        }
        if ((navMeshAgent.speed == speedRun) && !isRunning)
        {
    ...
    private void Patroling()
    {
        ...
        else
        {
            ...
            if (navMeshAgent.remainingDistance <= navMeshAgent.stoppingDistance)
            {
                ...
                if (m_WaitTime <= navMeshAgent.stoppingDistance)
                {
                    NextPoint();
                    Move(speedWalk);
                    m_WaitTime = startWaitTime;
    ...
    void EnviromentView()
    {
         ...
         * */
        Collider[] playerInRange = Physics.OverlapSphere(transform.position, viewRadius, TargetMask);   //  Make an overlap sphere around the enemy to detect the TargetMask in the view radius
        ...
        for (int i = 0; i < playerInRange.Length; i++)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToPlayer) < viewAngle / 2)
            {
                ...
                if (!Physics.Raycast(transform.position, dirToPlayer, dstToPlayer, obstacleMask))
                {
                     ...
                     * */
                    m_playerInRange = true;             //  The player has been seen by the enemy and then the nemy starts to chasing the player
    ...
    void MeshEnviromentView()
    {
       ...
       // viewMeshFilter.vertices = vertices;     //  Assign the created vertices to the vertices of the mesh filter
        ...
        //.RecalculateNormals();    //  Recalculated normal to show the mesh in the correct position
    }
